...
class InfluxManage(Ui_MainWindow, QObject):
    signal = pyqtSignal()
    ...
    def __init__(self):
        ...
        self.main_ui.actionxinjian.triggered.connect(self.new_connect)
        ...
        self.main_ui.execAction.triggered.connect(lambda: self.signal.emit())
        self.main_ui.history_button.triggered.connect(self.show_history)
        self.main_ui.tabWidget.tabCloseRequested.connect(self.tab_close)

        self.main_ui.treeView.setContextMenuPolicy(Qt.CustomContextMenu)  # 打开右键菜单的策略
    ...
    def save_connect(self, types):
        ...
        if types == 1:
            old_name = self.main_ui.treeView.currentItem().text(0)  # 取当前节点的value值
    ...
    def get_server_list(self, types=False, ID=None, rootIndex=None):
        ...
        for x in cursor:
            if types:
                ...
                root = QTreeWidgetItem(self.main_ui.treeView)
            root.setText(0, x[0])
            ...
            root.setIcon(0, icon)
            if types:
                self.main_ui.treeView.insertTopLevelItem(rootIndex, root)
                self.main_ui.treeView.setCurrentItem(root)

        self.main_ui.treeView.doubleClicked.disconnect()
        self.main_ui.treeView.doubleClicked.connect(self.double_handler)
    ...
    def create_table(self, text, name, database, tables_name=None):
        ...
        self.main_ui.tabWidget.addTab(tab, "{}.{}".format(name, database))
        current_index = self.main_ui.tabWidget.count()  # 获取tab数量
        self.main_ui.tabWidget.setCurrentIndex(current_index - 1)  # 激活最后一个创建的tab

    def group_select(self, series):
        current_text = self.QComboBox.currentText()
        currentTableWidget = self.main_ui.tabWidget.currentWidget().findChild(QTableWidget, "tableWidget")
        for x in series:
            tags = x.get('tags')
    ...
    def show_table(self, tables):
        ...
        try:
            ...
            for x in series:
                ...
                if tags:
                    if flag:
                        self.QComboBox.currentIndexChanged.connect(lambda: self.group_select(series))
                        flag = False
                    for n in tags:
                        self.QComboBox.addItem(tags[n])
                currentTableWidget.setColumnCount(len(columns))  # 设置列
                currentTableWidget.setRowCount(len(values))  # 设置行
                currentTableWidget.setHorizontalHeaderLabels(columns)  # 设置标题
                # currentTableWidget.setEditTriggers(QAbstractItemView.NoEditTriggers)  # 关闭双击编辑
    ...
    def double_handler(self):
        ...
        # treeView层级处理
        index_row = -1
        item = self.main_ui.treeView.currentItem()
        parent = item.parent()
        if parent is None:
            index_top = self.main_ui.treeView.indexOfTopLevelItem(item)
        else:
            index_top = self.main_ui.treeView.indexOfTopLevelItem(parent)
        ...
        if level == (-1, index_row):  # 当双击表的时候操作的事件
            ...
            if childCount == 0:
                ...
                try:
                    ...
                    name = self.main_ui.treeView.currentItem().text(0)  # 取当前节点的value值
            ...
            childCount = self.main_ui.treeView.currentItem().childCount()
            if childCount == 0:
                name = self.main_ui.treeView.currentItem().parent().text(0)  # 取服务器名
                database = self.main_ui.treeView.currentItem().text(0)  # 取数据库名
    ...
    def exec_handler(self):
        try:
            ...
            self.main_ui.statusBar.showMessage("执行完毕,耗时:{}毫秒".format(int(now_time - old_time)))
            color = ["blue", "green"]
            random.shuffle(color)
            self.main_ui.statusBar.setStyleSheet("color:{}".format(color[0]))
    ...
    def action_handler_1(self, types):
        """服务器层操作：
            ...
            4.删除服务器
          """
        name = self.main_ui.treeView.currentItem().text(0)
        self.influxDbClient.create_client(name)
        clients = self.influxDbClient.clients.get(name)
        client = clients.get("client")
        if types == 1:
        ...
        if types == 2:
            ...
            child = self.main_ui.treeView.currentItem().childCount()
            while child:
                now_child = self.main_ui.treeView.currentItem().child(0)
                self.main_ui.treeView.currentItem().removeChild(now_child)
                child -= 1
            self.main_ui.treeView.currentItem().setIcon(0, QIcon('images/server_close.ico'))
    ...
    def action_handler_2(self, types):
        ...
        """
        name = self.main_ui.treeView.currentItem().parent().text(0)
        database = self.main_ui.treeView.currentItem().text(0)
        self.influxDbClient.create_client(name, database=database)
        clients = self.influxDbClient.clients.get(name)
        client = clients.get("client")
        if types == 0:
                ...
                for i in x:
                    for z in i:
                        if switch:
                            ...
                            self.show_table(new_tables.raw)
                            switch = False
                        self.QComboBox.addItem(str(i[z]))
            self.QComboBox.currentIndexChanged[str].connect(self.select)
        elif types == 2:
    ...
    def right_click_menu(self, pos):
        ...
        if level == (-1, index_row):  # 当右击表的时候
            try:
                ...
                contextMenu.exec_(self.main_ui.treeView.mapToGlobal(pos))  # 随指针的位置显示菜单
                contextMenu.show()  # 显示
            except Exception as e:
                print(e)
